---
title: "Seminar 1: Data visualization in R"
...
mtcars$cyl_f <- factor(mtcars$qsec)
ggplot(data = mtcars, mapping = aes(x = wt, y = mpg, color = qsec)) + 
  geom_point()+ggtitle("Color-based")

#ggplot(data = mtcars, mapping = aes(x = wt, y = mpg, shape = qsec)) + 
  #geom_point()+ggtitle("Shape-based")

```

```{r answer1.2 part2, fig.align='center', echo=FALSE, comment= ""}
cat("A diferent category is generated for each data entry, as each data point is a different number.
For the mapping to a shape the plot produces an Error, as qsec is a continuous variable")
```
&emsp;
</div>
...
Which is the `position` argument in the `ggplot2` bar plot that standardizes the bars to the same height? Update the plot above with the new position adjustment.

<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
```{r answer2.1, fig.align='center'}
ggplot(data = mtcars, mapping = aes(x = cyl, fill = am_f)) +
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
```{r answer2.1 part2, fig.align='center', echo=FALSE, comment= ""}
cat("To compare the total amount of cars per each cylinder category I would use the graph that has no specified position, only geom_bar().
To know the relative abudance of each type of transmission in relation to cylinders, the position=fill option would be the best one.")
```
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
```{r answer 3.1 iris-facets, fig.align='center'}
iris$Petal.Type[iris$Petal.Length >= 4 ] <- "Long"
iris$Petal.Type[iris$Petal.Length < 4 ] <- "Short"

#Separate as before + separate according to length
ggplot(data = iris, mapping = aes(x = Sepal.Width, fill = Species)) +
  geom_histogram(binwidth = 0.1) +
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
```{r answer 3.1 part2, fig.align='center', echo=FALSE, comment= ""}
cat("The best subplot configuration would be the wrap option as more space is occupied with useful data, the grid option creates 2 empty sublots, wasting space.")
```
&emsp;
</div>
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
```{r answer 4.1 part1.1, fig.align='center', echo=FALSE, comment= ""}
cat("The scale_fill_viridis palettes, as they are designed to be perceived by viewers with common forms of colour blindness")
```
  
```{r answer 4.1 part1.2, fig.align='center'}
plot4<-ggplot(data = iris, mapping = aes(x = Sepal.Width, fill = Species)) +
  geom_histogram(binwidth = 0.1) +
  facet_grid(Species ~ .) +
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
```{r answer 4.1 part2.1, fig.align='center'}
plot4+labs(title = "Iris sepal variation", x = "Sepal width", fill = "Species name", caption = "A plot with iris data", subtitle="3 species are represented in this plot", tag="Jan Izquierdo")
```

```{r answer 4.1 part2.2, fig.align='center', echo=FALSE, comment= ""}
cat("They are used to give additional info to plots")
```
...
```{r answer 4.1 part3.1, fig.align='center'}
ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length)) +
  geom_point() +
  theme_minimal()
```
...
```{r answer 4.1 part3.2, fig.align='center', echo=FALSE, comment= ""}
cat("The theme option that maximizes data-ink ratio is theme_minimal in my opinion, as it has no background or gray colorings, it only displays the basic information to make the graph understandable.")
```
...
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
```{r answer 5.1 part1}
ggsave(filename = "plot_ex5.png", plot = p, width = 6, height = 4)
#install.packages("svglite")
library(svglite)
ggsave(filename = "plot_ex5.svg", plot = p, width = 6, height = 4)

```

```{r answer 5.1 part2, echo=FALSE, comment= ""}
cat("The difference that is more obvious is the file size, vector files occupy much less disk space than raster files")
```
&emsp;
</div>
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
```{r answer 6.1, fig.align='center', message=FALSE}
plot6<-ggplot(data = economics, mapping = aes(x = date, y = unemploy)) +
  geom_line()+
  labs(title = "Unemployment rates over time", x="Years", y="Unemployed individuals")+
  theme_classic() #Initially theme_minimal()

ggsave(filename = "plot6.svg", plot=plot6)
```
&emsp;
</div>
