...
```
```{r Task1, echo=F}
cat("Positive class is centred at coordinates (3,2)
...
# generate data from negative class
class_neg <- rmvnorm(n=225, mean=mean_neg, sigma=diag(1.5^2)) %>%  #MODIFIED
                    as_tibble() %>%
                    mutate(y=-1) %>%
                    rename(x1=V1, x2=V2)

# generate data from positive class
class_pos <- rmvnorm(n=225, mean=mean_pos, sigma=diag(1.5^2)) %>%  #MODIFIED
                    as_tibble() %>%
                    mutate(y=1) %>%
                    rename(x1=V1, x2=V2)
...
ggplot(data=data_gauss) + 
    geom_point(aes(x=x1, y=x2, color=y, shape=y))+
    theme(panel.background = element_blank())+ggtitle("Task1")
```
...
```{r Task2, echo=F}
cat("Calculate the correlation coefficient for the covariance matrix. Modify, the covariance matrix such
that the correlation coefficient is: i) -0.8, ii) -0.5, iii) 0, ii) +0.5, iii) +0.8. Do so while keeping the
standard deviation of each class and dimension equal to 1.5.")

# The classes are generated from a two dimensional gaussian with means (-2,0) and (2, 0). The covariance matrix is no longer diagonal
...
create_cov_matrix <- function(corr, sd_x, sd_y) {
  ...
  ggplot(data=data_skew) + 
      geom_point(aes(x=x1, y=x2, color=y, shape=y)) +
      theme(panel.background = element_blank())+ggtitle(paste0("Plot correlation coefficient= ", corr)) 
}
...
```{r Task3, echo=F}
cat("Modify the code such that the positive class has a correlation coefficient of 0.6 and the negative
class is -0.6.")
...
# covariance matrix for both classes
cov_matrix_neg = matrix(c(1, -.6, 
                          -.6, 1), nrow=2) #MODIFIED
cov_matrix_pos = matrix(c(1, .6,
                          .6,1), nrow=2) #MODIFIED
...
```
```{r Task4, echo=F}
cat("Modify the code such that the length for the negative class goes from 0.6 to 1.4 and the positive
class from 1 to 3.")
...
length_neg <- runif(n=200, min=0.6, max=1.4) #MODIFIED
length_pos <- runif(n=200, min=1, max=3) #MODIFIED
...
```{r Task5, echo=F, warning=FALSE}
cat("Change the position of the test point to (-0.8, -0.3) and see the new classification.
Are the observed means equal to the true values? Why? In case you have few examples, will this
impact the performance of the classifier? What is the reason for this change in performance?
Modify the distance, in a way that computes the sum of the absolute differences in both dimensions.
Does the classifier still work properly? Look at the decision boundary: is this a linear classifier?")
...
cat("The observed means are differnt to the true values but not by much, as they are near the cluster. The model is not complex enough to generate 
too many clusters, which would make it more accurate.\n\n")
cat("If we increased the complexity of the classifier to get more clusters it would be slower.\n\n")
...
```{r Task6, echo=FALSE}
cat("Modify the code to generate new test data from the same distribution as the training data and
calculate the error rate accordingly. You will need to investigate the code of the function:
get_nearest_centroid_predictions to use the function properly.")
```

```{r Task6.1, echo=F, warning=F}
plot_md_predictions(data_gauss, title='Gaussian point clouds')
...
predictions <- get_nearest_centroid_predictions(data_gauss, test_data)
```

```{r Task6.2, echo=F, warning=F}

```

```{r Task6.3, echo=F, warning=F}

```

```{r Task6.4, echo=F, warning=F}

```

```{r Task6.5, echo=F, warning=F}

```

```{r Task6.6, echo=F, warning=F}
...
```{r Task7, echo=F, warning=F}
cat("Modify the code to experiment with different values of k. Observe the effect on the decision
boundaries. For the case of the Gaussian mixture model, optimize manually the value of k that
minimizes the error in 200 test points generated with the same distribution.")
```
