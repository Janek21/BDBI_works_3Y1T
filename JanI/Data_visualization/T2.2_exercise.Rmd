---
title: "**T2.2** | Tools for data visualization"
subtitle: "In bioinformatics"
...
knitr::opts_chunk$set(echo = FALSE, eval = TRUE, fig.width = 4.5, fig.height = 3.5, fig.show = 'hold', message = FALSE, warning = FALSE)
library(ggplot2)
```
...
- __Quantitative and qualitative data__: scatterplots, barplots, boxplots, heatplots, ...
...
#### <i class="fas fa-pencil-alt"></i> **Exercise** | Go to the latest research biology articles from [Nature Communications](https://www.nature.com/search?subject=biological-sciences&journal=ncomms) journal and describe the types of visualizations used. How far can you enlarge the previous list?

<div style="background-color:#F0F0F0">
&emsp;<i class="fas fa-comment-dots"></i>
Answer:
```{r Ex1, echo=F}
cat("Boxplot, statistical test, pairwise comparison, Manhattan plot, and many others\n")
```
...
```{r echo=TRUE}
data <- read.table(file = "https://raw.githubusercontent.com/marta-coronado/data/refs/heads/main/expression_data.txt", #expression_data.txt
                   header = TRUE, sep ="\t",
...
ggplot(data=data, aes(x=gene_type, y=median_expression))+
...
ggplot(data[data$expressed,], aes(tissue))+geom_bar()
...
#ggplot(data=data, aes(x=tissue, y=median_expression))+
  #geom_col()+
  #scale_y_log10()+
  #geom_col(data=data_gene, color="red") #Very low expression points for data_gene

ggplot(data=data, aes(tissue, median_expression))+
  geom_boxplot()+ #Better than col, it shows the distribution
  scale_y_log10()+
...
ggplot(data=data_brain, aes(x=gene_chr, y=median_expression, color=median_expression>20000))+
  geom_point()

```
```{r Ex2.4.1, echo=FALSE}
cat("At the mitochondrial genome.\n")
```
...
Answer:
```{r Ex3, echo=FALSE}
cat("We can see different extensions in ggplot such as gganimate, ggseqlogo, ggupset, gganatogram, gggenes, ggQC, gg3D, ggtree and ggedit.
For htmlwidgets we can use plotly, heatmaply, diagrammeR, networkD3")
```
...
Examples by González lab at Institute of Evolutionary Biology: [DrosOmics](https://www.biologiaevolutiva.org/gonzalez_lab/drosomics/?session=local-953bZ0HN-) 
...
### Upload `T2.2_slides.Rmd` with the completed exercises (text included) to [aul@-ESCI](https://aula.esci.upf.edu/course/view.php?id=8592)
