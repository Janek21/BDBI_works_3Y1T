...
library(MASS)
set.seed(2)
n_samples <- 400 # 400 items, what we want to predict 
n_features <- 10 # simple to see if it is working or not , ind variables, variables we use to predict the independent values 
...
for (cat in categories) {
  ...
  for (pat in 1:n_patterns) {
    n_ids_cate_with_pattern <- sum(pattern==pat) #count num equal patterns to find important
    mus <- rnorm(n_features, mean = 0, sd= 5 ) # the sd is large because that way the knn will be confused and will focus on the ones we want 
    X <- mvrnorm(n_ids_cate_with_pattern, mu = mus, Sigma = diag(n_features)) #covariance matrix of 0s and 1s
...
random_features <- 1000 - n_features #take 990 -> 1000-10, we want to add 990 cols to the existing matrix
random_mean <- mean(m)
...
create_population<-function(N, chr_len, total_chr_ind){
  ...
  for (i in 1:N){ #create the population
    chr<-sample(total_chr_ind, chr_len)#get chr_len number of chromosomes from the total
    population[[i]]<-chr #save the chromosomes in a list in the total population list
  }
  return(population)
}
