...
x and y coordinates in the graph

x and y variables for css, image, mapping and image to css ratio.

And x and y variables for domain, range and log (which are null in our example)



For the brush action:

Xmin, Xmax, Ymin and Ymax for position coordinates

Xmin, Xmax, Ymin and Ymax as variables for coords_css and coords_img

x and y as variables for img_css_ratio, mapping and log (which are NULL in our case)

left, right, bottom and top, which are variables used for the domain and range options

And direction, brushID and outputID which have no sub-variables.
...
Increases the number of possible point selections within the threshold distance.
...
Yes you can interactively select the X axis limits I would use a slider input, as it allows to select all values between x1 and x2.
