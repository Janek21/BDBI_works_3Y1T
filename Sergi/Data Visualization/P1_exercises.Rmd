---
title: "**Data visualization in R**<br><font color='#A8A8A8' size='5'>First steps with `ggplot2`</font><img src='img/logoesci-fons-blanc.png' style='position:absolute;top:0px;left:0px;' width='10%'/>"
author: "Marta Coronado Zamora and Adrià Auladell"
...
# Mapping qsec to colour
ggplot(data = mtcars, mapping = aes(x = wt, y = mpg, colour = qsec)) + 
  geom_point()

# Mapping qsec to shape (will likely cause an error)
...
# Standardize the bars using position = "fill"
ggplot(data = mtcars, mapping = aes(x = cyl, fill = am_f)) +
...
```{r}
#compare total amounts in each cylinder category
ggplot(data = mtcars, mapping = aes(x = cyl, fill = am_f)) +
  geom_bar(position = "dodge")

# Stacked bars with same height to compare relative abundance
ggplot(data = mtcars, mapping = aes(x = cyl, fill = am_f)) +
  geom_bar(position = "fill")
...
iris$Petal.Type <- ifelse(iris$Petal.Length >= 4, "Long", "Short")

#Species and Petal.Type using a grid
ggplot(data = iris, mapping = aes(x = Sepal.Width, fill = Species)) +
  geom_histogram(binwidth = 0.1) +
  facet_grid(Species ~ Petal.Type)


#Petal.Type using facet_wrap
ggplot(data = iris, mapping = aes(x = Sepal.Width, fill = Species)) +
  geom_histogram(binwidth = 0.1) +
...
```{r}
cat("Depends on what you are looking for exactly, for comparison between species,
the grid is better, but with the wrap we can easily see the overall distribution.")
```
...
```{r}
# Using the scale_fill_viridis_d
ggplot(data = iris, mapping = aes(x = Sepal.Width, fill = Species)) +
  geom_histogram(binwidth = 0.1) +
  facet_grid(Species ~ .) +
  scale_fill_viridis_d() +
  ggtitle("Iris Sepal Width Distribution in Viridis Palette")

# Using the Set1 from scale_fill_brewer 
ggplot(data = iris, mapping = aes(x = Sepal.Width, fill = Species)) +
  geom_histogram(binwidth = 0.1) +
  facet_grid(Species ~ .) +
  scale_fill_brewer(palette = "Set1") +
  ggtitle("Iris Sepal Width Distribution with Set1")
...
ggplot(data = iris, mapping = aes(x = Sepal.Width, fill = Species)) +
  ...
  scale_fill_brewer(palette = "Set1") +
  labs(
    title = "Iris Sepal Width Distribution",
    subtitle = "By Species",
    caption = "Data source: Iris Data",
    tag = "Figure 1"
  )

cat("The functions are from the ggplot2 package in order to add some more 
informative text in a plot ")
```
