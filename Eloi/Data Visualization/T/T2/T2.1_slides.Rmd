...
```{r}
# Answer
#Size, position and gradient of transparency or color.For discrete variables all of them are good even though for some of those like size or transparency when the differences are very subtle then maybe they are not noticeable.
```
...
ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length, color = Species)) +
  geom_point() +
  scale_color_manual(values = c("darkorange", "steelblue", "green"))

ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length)) +
  geom_point(color = "darkorange", alpha = 0.5)

ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length, size = Petal.Length)) +
  geom_point(color = "darkorange")

ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length, size = Petal.Length, color = Species)) +
...
#Yes, they are grouped in 6 groups using 2 discrete variables, automatic (0,1) and num of cilinders.
```
...
#Answer: geom_line, geom_area, geom_poligon, geom_path
...
Arrows, circles, colors, using a rectangle
...
p2 <- ggplot(msleep, aes(x = vore, y = sleep_total, colour = vore, fill = vore)) +
  geom_violin(alpha = 0.4, colour = NA) +  # No outline for violins
  geom_jitter(width = 0.3, size = 2, alpha = 0.8) +  # Jitter points
  theme_minimal(base_size = 14) +  # Minimal theme with larger base size
  scale_fill_brewer(palette = "Spectral") +  # Use a vibrant color palette
  labs(title = "Sleep Patterns by Vore Category",
       x = "Vore Category",
       y = "Total Sleep (hours)") +
  theme(
    ...
    panel.grid.major = element_line(size = 0.2, linetype = 'dotted', colour = "gray50"),  # Dotted major grid lines
    axis.ticks.y = element_blank(),  # No ticks on y-axis
    axis.text.x = element_text(angle = 45, hjust = 1, face = "bold", size = 10, colour = "gray50"),  # Smaller x-axis text
    axis.text.y = element_text(size = 10, colour = "gray50"),  # Smaller y-axis text
    plot.title = element_text(hjust = 0.5, face = "bold", size = 16),  # Bold, centered title
    plot.subtitle = element_text(hjust = 0.5, size = 12, colour = "gray50")  # Subtitle
  )
