...
- Theory 
- Examples and short exercises in class (<i class="fas fa-pencil-alt"></i>)  - complete and submit to [aul@-ESCI](https://aula.esci.upf.edu/course/view.php?id=8592)
...
```{r Ex1.1, echo=FALSE, comment=""}
# Answer
cat("A continuous variable can be mapped to:\nPosition\n")
```
No: shape
```{r Ex1.1 plots}
ggplot(data=iris, aes(x=Sepal.Width, y=Sepal.Length, colour=Petal.Width))+
  geom_point()+labs(title="Color", x="Sepal width", y="Sepal length", colour="Petal width")

ggplot(data=iris, aes(x=Sepal.Width, y=Sepal.Length, alpha=Petal.Width))+
  geom_point()+labs(title="Transparency", x="Sepal width", y="Sepal length", alpha="Petal width")

ggplot(data=iris, aes(x=Sepal.Width, y=Sepal.Length, size=Petal.Width))+
  geom_point()+labs(title="Size", x="Sepal width", y="Sepal length", size="Petal width")
```

```{r Ex1.2, echo=FALSE, comment=""}
# Answer
cat("A discrete variable can be mapped to:\nPosition\n")
```
```{r Ex1.2 plots, warning=FALSE}
ggplot(data=iris, aes(x=Sepal.Width, y=Sepal.Length, colour=Species))+
  geom_point()+labs(title="Color", x="Sepal width", y="Sepal length")

ggplot(data=iris, aes(x=Sepal.Width, y=Sepal.Length, shape=Species))+
  geom_point()+labs(title="Shape", x="Sepal width", y="Sepal length")

ggplot(data=iris, aes(x=Sepal.Width, y=Sepal.Length, group=Species, linetype=Species))+
  geom_line()+labs(title="Linetype", x="Sepal width", y="Sepal length")

ggplot(data=iris, aes(x=Sepal.Width, y=Sepal.Length, alpha=Species))+
  geom_point()+labs(title="Transparency", x="Sepal width", y="Sepal length", subtitle = "Warning: not advised")

ggplot(data=iris, aes(x=Sepal.Width, y=Sepal.Length, size=Species))+
  geom_point()+labs(title="Size", x="Sepal width", y="Sepal length", subtitle = "Warning: not advised")
```
...
Answer:
```{r Ex2.1, echo=FALSE}
cat("We can use what we learned from the experimentation in the previous exercise to create a plot representing all characteristics at once")
```

```{r Ex2.2}
# Answer
ggplot(data=iris, aes(x=Sepal.Width, y=Sepal.Length, colour=Petal.Length, shape=Species, alpha=Petal.Width))+
  geom_point()+labs(title="Color", x="Sepal width", y="Sepal length", colour="Petal length", shape="Petal width")+
  theme(legend.key.size = unit(0.4, "cm"), legend.text = element_text(size = 9), legend.title = element_text(size = 10))
```
&emsp;
</div>
...
cat("Simplified:")
ggplot(mpg)+ #ggplot(mpg, aes(x=displ, y=hwy))
  geom_point(aes(x=displ, y=hwy)) #or aes(displ, hwy)
...
cat("Simplified:")
ggplot(msleep, aes(x = log(brainwt), y = log(bodywt))) +
  geom_point()
```
...
```{r Ex4, echo=FALSE}
cat("Yes, the observations are joint in 6 groups, 3 main groups by to cylinders(4,6 and 8) and each one of those is split by manual or automatic transmission")
```
...
```{r Ex5.1, echo=FALSE}
cat("The following geoms are collective:\n")
```
```{r Ex5.2}
p + geom_col(fill="gray") + labs(title = "geom_col")
p + geom_tile(fill="gray") + labs(title = "geom_tile")
p + geom_polygon(fill="gray") + labs(title = "geom_polygon")
p + geom_area(fill="gray") + labs(title = "geom_area")
```
...
```{r Ex6, echo=FALSE}
cat("A: geom_point, as a circle to highlight the points of interest.\n")
cat("B: geom_line, plotting the expected trend.\n")
cat("C: geom_text, as it allows for writing text")
...
### Upload `T2.1_slides.Rmd` with the completed exercises (text included) to [aul@-ESCI](https://aula.esci.upf.edu/course/view.php?id=8592)
